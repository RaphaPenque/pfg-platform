...
export function registerRoutes(server: Server, app: Express) {
  ...
  // ===== TEMPORARY: OEM SPOT CHECK (remove after use) =====
  app.get("/api/debug/oem-check", requireRole("admin"), async (_req: Request, res: Response) => {
    try {
      ...
      const stats = await client.query(`
        SELECT 
          (SELECT COUNT(*) FROM oem_experience) as total_entries,
          (SELECT COUNT(DISTINCT worker_id) FROM oem_experience) as workers_with_rel,
          (SELECT COUNT(*) FROM workers WHERE oem_experience IS NOT NULL AND oem_experience NOT IN ('[]','null','')) as workers_with_legacy
      `);

      const dupes = await client.query(`
        SELECT w.id, w.name, oe.oem, oe.equipment_type, COUNT(*) as cnt
        FROM workers w JOIN oem_experience oe ON oe.worker_id = w.id
        GROUP BY w.id, w.name, oe.oem, oe.equipment_type
        HAVING COUNT(*) > 1 ORDER BY cnt DESC
      `);

      const gaps = await client.query(`
        SELECT w.id, w.name, w.oem_experience
        FROM workers w LEFT JOIN oem_experience oe ON oe.worker_id = w.id
        WHERE w.oem_experience IS NOT NULL AND w.oem_experience NOT IN ('[]','null','')
          AND oe.id IS NULL ORDER BY w.id
      `);

      const mixed = await client.query(`
        SELECT DISTINCT w.id, w.name, w.status FROM workers w
        JOIN oem_experience oe1 ON oe1.worker_id = w.id 
        JOIN oem_experience oe2 ON oe2.worker_id = w.id
        WHERE (oe1.equipment_type ILIKE '%GT%' AND (oe2.equipment_type ILIKE '%ST%' OR oe2.equipment_type ILIKE '%CC%'))
           OR (oe1.equipment_type ILIKE '%ST%' AND oe2.equipment_type ILIKE '%CC%')
        ORDER BY w.id LIMIT 8
      `);

      const topCount = await client.query(`
        SELECT w.id, w.name, w.status, COUNT(oe.id) as cnt FROM workers w
        JOIN oem_experience oe ON oe.worker_id = w.id
        GROUP BY w.id, w.name, w.status ORDER BY cnt DESC LIMIT 8
      `);

      const checkSet = new Map();
      mixed.rows.forEach((r: any) => checkSet.set(r.id, r));
      topCount.rows.forEach((r: any) => { if (!checkSet.has(r.id)) checkSet.set(r.id, r); });
      const checkIds = [...checkSet.keys()].slice(0, 10);

      const detail = await client.query(
        `SELECT w.id, w.name, w.status, w.oem_experience as legacy,
                oe.id as oe_id, oe.oem, oe.equipment_type, oe.years_experience
         FROM workers w LEFT JOIN oem_experience oe ON oe.worker_id = w.id
         WHERE w.id = ANY($1) ORDER BY w.id, oe.oem, oe.equipment_type`,
        [checkIds]
      );

      const byWorker: Record<number, any[]> = {};
      const workerMeta: Record<number, any> = {};
      detail.rows.forEach((r: any) => {
        workerMeta[r.id] = { name: r.name, status: r.status, legacy: r.legacy };
        if (r.oe_id) {
          if (!byWorker[r.id]) byWorker[r.id] = [];
          byWorker[r.id].push({ oem: r.oem, equip: r.equipment_type, years: r.years_experience });
        }
      });

      const equipDist = await client.query(`SELECT equipment_type, COUNT(*) as cnt FROM oem_experience GROUP BY equipment_type ORDER BY cnt DESC LIMIT 15`);
      const oemDist = await client.query(`SELECT oem, COUNT(*) as cnt FROM oem_experience GROUP BY oem ORDER BY cnt DESC LIMIT 15`);
      ...
      res.json({
        ...
        migrationGaps: gaps.rows.map((r: any) => ({ id: r.id, name: r.name, legacyCount: (() => { try { return JSON.parse(r.oem_experience).length; } catch(e) { return -1; } })() })),
        spotCheck: checkIds.map(wid => {
          const meta = workerMeta[wid] || {};
          const entries = byWorker[wid] || [];
          const oemGroups: Record<string, Set<string>> = {};
          const seen = new Set<string>();
          const dupeKeys: string[] = [];
          entries.forEach((e: any) => {
            const key = `${e.oem}|||${e.equip}`;
            if (seen.has(key)) dupeKeys.push(`${e.oem} + ${e.equip}`);
            seen.add(key);
            if (!oemGroups[e.oem]) oemGroups[e.oem] = new Set();
            oemGroups[e.oem].add(e.equip);
          });
          const pills = Object.entries(oemGroups).map(([oem, equipSet]) => `${oem}  ${[...equipSet].sort().join(' · ')}`);
          return { id: wid, name: meta.name, status: meta.status, entryCount: entries.length, pills, dupes: dupeKeys };
        }),
        equipmentDistribution: equipDist.rows,
        oemDistribution: oemDist.rows,
      });
    } catch(err: any) {
