...
<body style="font-family:'Inter',Arial,sans-serif;background:#f4f4f5;padding:40px 0;">
  ...
  // ONE-TIME: Insert Andrea Marchionni's Saltend survey response (wiped in data loss)
  // DELETE THIS ROUTE AFTER USE
  app.post("/api/admin/insert-andrea-survey", requireAuth, requireRole("admin"), async (req: Request, res: Response) => {
    try {
      const crypto = await import('crypto');
      const token = crypto.randomBytes(32).toString('hex');
      const expiresAt = new Date(Date.now() + 60 * 1000); // expires in 60s

      // Create a survey token for Andrea (already submitted 16 Apr 2026)
      const surveyToken = await storage.createSurveyToken({
        projectId: 10,
        token,
        contactEmail: 'andrea.marchionni.w5@mhi.com',
        contactName: 'Andrea Marchionni',
        expiresAt,
      });

      // Scores: q1=Planning(4), q2=Quality(5), q3=HSE(4), q4=Supervision(5), q5=PM(4), q6=Overall(5)
      const scores = [4, 5, 4, 5, 4, 5];
      ...
      const surveyResponse = await storage.createSurveyResponse({
        ...
        averageScore,
        nps: 9,
        openFeedback: "We are really pleased with the PowerForce Global manpower crew performance during 2026 GT2 CI. This was the first time we employed PFG for Saltend outages, and the result was very positive. The only point of improvement we would recommend is that all trainings and certificates if possible should be sorted within 2/3 weeks before the outage please. In SAL we need to issue a letter to customer (validation that all people are ok to work at site) 1 week before outage starting date, so certificates should not be issued within a week otherwise there is no time to properly check or assess them. Apart from that, the entire planning process was very professional and satisfactory. Stephen Haslam being on site was a real added value and we hope this solution will be available again for future outages.",
        individualFeedbackGiven: false,
      });

      // Mark token as used (submitted 16 Apr 2026)
      await storage.updateSurveyToken(surveyToken.id, { usedAt: new Date('2026-04-16T10:00:00Z') });

      return res.json({ ok: true, tokenId: surveyToken.id, responseId: surveyResponse.id, averageScore });
    } catch (e: any) {
