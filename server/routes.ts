...
export function registerRoutes(server: Server, app: Express) {
  ...
  app.use("/api", (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.path.startsWith("/auth/") || req.path.startsWith("/portal/") || req.path.startsWith("/api/portal/") || req.path.startsWith("/confirm/")) return next();
...
<body style="font-family:'Inter',Arial,sans-serif;background:#f4f4f5;padding:40px 0;">
  ...
  // ONE-OFF: Regenerate GRTY w/c 2026-04-13 weekly report PDF with corrected data
  app.post("/api/internal/regenerate-weekly-report", async (req: Request, res: Response) => {
    if (req.headers["x-api-key"] !== "pfg-internal-2026") {
      return res.status(401).json({ error: "Unauthorized" });
    }
    try {
      const { weeklyReportId } = req.body;
      if (!weeklyReportId) return res.status(400).json({ error: "weeklyReportId required" });

      // Load the weekly report row
      const wrRes = await db.execute(sql`
        SELECT wr.*, p.id as project_id_val, p.code, p.name as project_name,
               p.customer, p.site_name, p.start_date, p.end_date,
               p.contract_type, p.shift, p.equipment_type
        FROM weekly_reports wr
        JOIN projects p ON p.id = wr.project_id
        WHERE wr.id = ${weeklyReportId}
      `);
      if (!wrRes.rows.length) return res.status(404).json({ error: "Weekly report not found" });
      const wr = wrRes.rows[0] as any;

      const weekStart = wr.week_commencing;
      const weekEnd = wr.week_ending;
      const projectId = wr.project_id;

      // Re-fetch all source data
      const [assignments, allWorkers, allToolboxTalks, allSafetyObs, allIncidents, allComments, allDailyReports] = await Promise.all([
        storage.getAssignmentsByProject(projectId),
        storage.getWorkers(),
        storage.getToolboxTalks(projectId),
        storage.getSafetyObservations(projectId),
        storage.getIncidentReports(projectId),
        storage.getCommentsLog(projectId),
        storage.getDailyReports(projectId),
      ]);

      const workerMap = Object.fromEntries((allWorkers as any[]).map((w: any) => [w.id, w]));

      // Team members: active assignments overlapping the week
      ...
      // Published daily reports for the week
      const published = (allDailyReports as any[]).filter((r: any) => r.publishedToPortal);
      const weekReports = published.filter((r: any) => (r.reportDate || '') >= weekStart && (r.reportDate || '') <= weekEnd);

      // Delays with date stamped
      const reportDelays = weekReports.flatMap((r: any) =>
        Array.isArray(r.delaysLog)
          ? r.delaysLog.map((d: any) => ({ ...d, date: r.reportDate || '' }))
          : []
      );

      // Comments
      const weekReportIds = new Set(weekReports.map((r: any) => r.id));
      const commentsEntries = (allComments as any[]).filter((c: any) =>
        ((c.logDate || '') >= weekStart && (c.logDate || '') <= weekEnd) ||
      ...
      const safetyObservations = (allSafetyObs as any[]).filter((o: any) =>
        (o.observationDate || '') >= weekStart && (o.observationDate || '') <= weekEnd
      );
      const toolboxTalks = (allToolboxTalks as any[]).filter((t: any) =>
      ...
      const pStart = wr.start_date || weekStart;
      const pEnd = wr.end_date || weekEnd;
      const totalDays = Math.max(1, Math.round((new Date(pEnd).getTime() - new Date(pStart).getTime()) / 86400000));
      const elapsedDays = Math.max(0, Math.round((new Date(weekEnd).getTime() - new Date(pStart).getTime()) / 86400000));
      const daysRemaining = Math.max(0, Math.round((new Date(pEnd).getTime() - new Date(today).getTime()) / 86400000));
      ...
      const reportData = {
        projectName: wr.project_name,
        projectCode: wr.code,
        customer: wr.customer || '',
        siteName: wr.site_name || '',
        startDate: wr.start_date || '',
        endDate: wr.end_date || '',
        contractType: wr.contract_type || '',
        shiftPattern: wr.shift || '',
        weekStart,
        weekEnd,
        pmName,
        completedTasks: [],
      ...
      return res.json({
        ...
        obsCount: safetyObservations.length,
        tbtCount: toolboxTalks.length,
        delayCount: reportDelays.length,
        pdfPath,
      });
    } catch (err: any) {
      console.error('[regenerate-weekly-report] error:', err.message, err.stack);
