...
### 2026-04-27 (session close) — Session #6 outcomes summary

Documentation-only roll-up of work completed in Session #6. No application code changed in this entry. Source: `SESSION_HANDOFF_2026-04-27_SESSION6_CLOSE.md`.

- **GRTY timesheet recovery completed and verified.** Stale non-paid rows on GRTY w/c 2026-04-20 (entries `120370` `demob`, `120371` `rest_day`) had `time_in`/`time_out`/`total_hours` set to `NULL` under explicit approval; `day_type`, `unpaid_break_minutes=60`, `is_override=true`, week status, token, and timestamps preserved. `timesheet_weeks.approval_hash` and `approval_preimage` recomputed. Audit row `audit_logs.id=2259` written with `action="timesheet.recovery_correction"`. Post-fix verification: Goran paid hours `36.00`, row hours `36.00`, N2/N3 violations empty, hash self-check true. Health: `87 checks — 0 failed, 18 warnings, 69 passed`. No customer email, recall, resend, or approval action performed. Root cause was operational drift, not a current code regression.
- **GRTY draft preview regenerated safely.** From Weekly Ops on `GRTY / GE ST Great Yarmouth UK`, week commencing `20/04/2026`, used `Generate preview (no email)`. Draft `weekly_reports.id=2` remains draft/unsent; preview banner correct; period reads `20 April 2026 — DRAFT PREVIEW · NOT YET SENT`; project progress sensible (13%, 111 of 126 days remaining). Customer portal without `preview=1` continues to hide the draft. No emails sent, no customer-facing report published.
- **PR #9 — worker docs → customer portal Team SQEP workflow — merged and QA passed.** Merge commit `4a9a88ba6e9262555a3f9cc3d03e381f60f5fd28`. Cert uploads now upsert full document metadata; date-only document saves preserve existing file pointers; worker profile renders a passport download icon when `passportPath` is set; Customer Portal buttons open the tokenized portal in a new tab; Team SQEP zip includes reconciled certificate files via the read-only `reconcileDocsWithDisk` / `reconcileWorkerFilePaths` helpers. Smoke coverage 13/13 passing. Post-deploy health: `95 checks — 0 failed, 18 warnings, 77 passed`; Section O passes O1/O2/O4/O5/O6, O3 warning remains (2 legacy cert rows with dates but no `file_path`). Manual QA passed for Manuel Rabano (worker id `44`) and Ramon Alcaine (worker id `108`). Passports remain deliberately out of scope for the customer SQEP zip.
- **SALTEND project provenance.** Project id `28`, code `SALTEND`, customer Mitsubishi Power, dates 2026-05-04 to 2026-05-05, status `capacity_planning`, contract Time & Materials, no signatory/headcount/assignments at time of check. `audit_logs.id=2266`, `action="project.create"`, created 2026-04-27 15:39:08 UTC by user id `3` (Wesley Martin, role `project_manager`). Distinct from the older completed project id `10` (`SALT`, "Mitsubishi GT Saltend UK").
- **PR #10 — `Approve without Supervisor` controlled override — merged and post-deploy health passed.** Merge commit `f3e104df90ae72d4286c34c6aad5a78fc0c03103`, merged 2026-04-27T17:30:33Z. Endpoint `POST /api/weekly-ops/approve-without-supervisor`, role-gated (admin/PM/RM), reason+evidence+dual-ack required, status guard (`draft`|`submitted`), refused if any supervisor submission exists, refused for `customer_signoff_required=false`, atomic conditional UPDATE before audit, calendar-date validation, reason/evidence length caps, audit metadata captures actor email/role/IP/UA, never calls `sendMail`, status always `pm_approved`. Schema added four nullable columns to `timesheet_weeks` via idempotent `ADD COLUMN IF NOT EXISTS`. Two independent reviews (Codex + Claude) cleared after patch commit `fe87b6e`. Pre-merge: `workflow-invariants` 15/15, `timesheet-hours` 25/25, `report-period` pass, client typecheck clean. Post-deploy health: `96 checks — 0 failed, 18 warnings, 78 passed`; N13 passes all twelve sub-checks (role, reason, evidence, dual-ack, audit, status guard, no-customer-email, calendar date, length caps, customer-signoff guard, conditional update, IP/user-agent). Remaining warnings are pre-existing/operational.
- **Next action — gated.** Identify a safe override QA candidate from read-only SQL (`status IN ('draft','submitted')`, no day/night supervisor submission, `pm_approve_override_at IS NULL`, not customer-sent/approved/recalled, `customer_signoff_required=true`) and present candidates for explicit approval **before** any production override write. A successful manual QA mutates a real production timesheet week — do not perform it without explicit per-project/per-week approval and approved reason/evidence text. Safer fallback is UI-only QA (open the modal, cancel before submission). Order of business after override QA is settled: timesheet tab workflow card, then reports tab workflow card.
